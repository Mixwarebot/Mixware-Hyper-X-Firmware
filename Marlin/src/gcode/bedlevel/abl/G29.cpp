...
#endif
#include "../../../lcd/extui/dgus/mks/MixwareUI.h"
...
G29_TYPE GcodeSuite::G29() {
  ...
  if (homing_needed_error()) {DGUS_LCD_UI_MIXWARE
    IF_ENABLED(TFT_MIXWARE_LVGL_UI, mixware_ui.set_auto_leveling_state(LEVEL_STATE_LEVELERR));
    IF_ENABLED(DGUS_LCD_UI_MIXWARE, mix_level.set_auto_leveling_state(LEVEL_STATE_LEVELERR));
    G29_RETURN(false);
  }
  ...
  if (!g29_in_progress) {
    ...
    TERN_(HAS_MULTI_HOTEND, if (!active_extruder) tool_change(1));
    ...
    abl.dryrun = parser.boolval('D') || TERN0(PROBE_MANUALLY, no_action);
      ...
      const float x_min = probe.min_x(), x_max = (X2_MAX_POS) - (PROBING_MARGIN_RIGHT),//probe.max_x(),
  ...
  IF_ENABLED(DGUS_LCD_UI_MIXWARE, if (mix_level.get_auto_leveling_state() == LEVEL_STATE_LEVELING) mix_level.set_auto_leveling_state(LEVEL_STATE_FINISHED));
