...
void GcodeSuite::M106() {
  ...
  TERN_(MIXWARE_HYPER_X, if (pfan == 2) thermalManager.set_fan_speed(3, thermalManager.fan_speed[pfan]));
  TERN_(MIXWARE_HYPER_X, if (pfan == 3) thermalManager.set_fan_speed(2, thermalManager.fan_speed[pfan]));
  TERN_(MIXWARE_HYPER_X, thermalManager.print_fan_speed());
}
...
void GcodeSuite::M107() {
  ...
  TERN_(MIXWARE_HYPER_X, if (pfan == 2) thermalManager.set_fan_speed(3, 0));
  TERN_(MIXWARE_HYPER_X, if (pfan == 3) thermalManager.set_fan_speed(2, 0));
  TERN_(LASER_SYNCHRONOUS_M106_M107, planner.buffer_sync_block(BLOCK_BIT_SYNC_FANS));
}
