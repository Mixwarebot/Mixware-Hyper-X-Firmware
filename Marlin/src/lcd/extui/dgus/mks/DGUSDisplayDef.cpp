...
void MKS_reset_settings() {
  constexpr xy_int_t init_dgus_level_offsets[5] = {
    { 20, 20 }, { X_MAX_POS - 20, 20 },
    { 20, Y_MAX_POS - 20 }, { X_MAX_POS - 20, Y_MAX_POS - 20 },
    { X_CENTER, Y_CENTER }
  };
  mks_language_index = 0;
  COPY(mks_corner_offsets, init_dgus_level_offsets);
  mks_park_pos.set(2, 0, 15);
  mks_min_extrusion_temp = 0;

  DGUSDetectorFil_L = false;
  DGUSDetectorFil_R = false;
  DGUSTEMP_Protect = false;
  e0tempmode = 0;
  e1tempmode = 0;
}
...
void MKS_pause_print_move() {
  ...
  // Save the current position, the raise amount, and 'already raised'
    ...
    if (recovery.enabled) {
      float print_paused_raised = mks_park_pos.z;
      if (position_before_pause.z + mks_park_pos.z > Z_MAX_POS)
        print_paused_raised = Z_MAX_POS - position_before_pause.z;
...
const uint16_t VPList_Main[] PROGMEM = {
  // VP_M117, for completeness, but it cannot be auto-uploaded.
    ...
    VP_T_Chamber_Is, VP_T_Chamber_Set, VP_Chamber_STATUS,
  #endif
  #if HAS_FAN
...
const uint16_t MKSList_PREHEAT[] PROGMEM = {
  // E Temp
  REPEAT(EXTRUDERS, MKSLIST_E_ITEM)
  // HB Temp
  VP_T_Bed_Is, VP_T_Bed_Set,
...
const uint16_t MKSList_FAN[] PROGMEM = {
  // E Temp
  REPEAT(EXTRUDERS, MKSLIST_E_ITEM)
  // HB Temp
  VP_T_Bed_Is, VP_T_Bed_Set,
...
const uint16_t MKSList_REPLACEFIL[] PROGMEM = {
  // E Temp
  REPEAT(EXTRUDERS, MKSLIST_E_ITEM)
  // HB Temp
  VP_T_Bed_Is, VP_T_Bed_Set,
...
const uint16_t MKSList_Jerk[] PROGMEM = {
  // E Temp
  REPEAT(EXTRUDERS, MKSLIST_E_ITEM)
  // HB Temp
  VP_T_Bed_Is, VP_T_Bed_Set,
...
const struct VPMapping VPMap[] PROGMEM = {
  ...
  { MKSLCD_AUTO_LEVEL2, MKSAuto_Level },                       // Page 73
  { MKSLCD_Screen_Offset_Config, MKSOffset_Config },          // Page 30
  { MKSLCD_Screen_Advance_Config, MKSOffset_Config },          // Page 110
  { MKSLCD_Screen_PMove, MKSList_MOVE },                      // Page 64
  { MKSLCD_Screen_Baby, MKSBabyStep },                        // Page 71
  { MKSLCD_Screen_Baby2, MKSBabyStep },                        // Page 71
  { DGUSLCD_SCREEN_PREHEAT, MKSList_PREHEAT },                // Page 130,
  { DGUSLCD_SCREEN_TEMPERATURE_1, MKSList_Home },             // Page 69
  { DGUSLCD_SCREEN_TEMPERATURE_2, MKSList_Home },             // Page 71
  { DGUSLCD_SCREEN_TEMPERATURE_3, MKSList_Home },             // Page 74
  { DGUSLCD_SCREEN_TEMPERATURE_4, MKSList_Home },             // Page 76
  { DGUSLCD_SCREEN_FAN_LIGHT_1, MKSList_FAN },                // Page 61,
  { DGUSLCD_SCREEN_FAN_LIGHT_2, MKSList_FAN },                // Page 63,
  { DGUSLCD_SCREEN_FAN_LIGHT_3, MKSList_FAN },                // Page 65,
  { DGUSLCD_SCREEN_FAN_LIGHT_4, MKSList_FAN },                // Page 67,
  { DGUSLCD_SCREEN_ACCELERATION, MKSList_MotoConfig },        // Page 113,
  { DGUSLCD_SCREEN_JERK_SPEED, MKSList_Jerk },                // Page 114,
...
const char H43Version[] PROGMEM = "Mixware Hyper X v1.0.07";
...
const struct DGUS_VP_Variable ListOfVP[] PROGMEM = {
  ...
  VPHELPER(VP_MOVE_X2, nullptr, ScreenHandler.HandleManualMove, nullptr),
  ...
  // Temperature Data
    ...
    VPHELPER(VP_E0_STATUS, &thermalManager.temp_hotend[0].target, nullptr, ScreenHandler.DGUSLCD_SendHeaterStatusToDisplay),
      ...
      VPHELPER(VP_RELOADING_IN, nullptr, ScreenHandler.MKS_ReloadingIn, nullptr),
      VPHELPER(VP_RELOADING_OUT, nullptr, ScreenHandler.MKS_ReloadingOut, nullptr),
    #endif
  #endif
    ...
    #if ENABLED(PIDTEMP)
      VPHELPER(VP_E1_PID_P, &thermalManager.temp_hotend[1].pid.Kp, ScreenHandler.HandleTemperaturePIDChanged, ScreenHandler.DGUSLCD_SendTemperaturePID),
      VPHELPER(VP_E1_PID_I, &thermalManager.temp_hotend[1].pid.Ki, ScreenHandler.HandleTemperaturePIDChanged, ScreenHandler.DGUSLCD_SendTemperaturePID),
      VPHELPER(VP_E1_PID_D, &thermalManager.temp_hotend[1].pid.Kd, ScreenHandler.HandleTemperaturePIDChanged, ScreenHandler.DGUSLCD_SendTemperaturePID),
  ...
  VPHELPER(VP_PREHEAT_BUTTON, nullptr, ScreenHandler.PREHEAT_CTRL, nullptr),
  VPHELPER(VP_TEMPERATURE_BUTTON, nullptr, ScreenHandler.Temperature_Ctrl, nullptr),
  VPHELPER(VP_FAN_LIGHT_BUTTON, nullptr, ScreenHandler.Fan_Light_Ctrl, nullptr),
  VPHELPER(VP_BABYSTEP_BUTTON, nullptr, ScreenHandler.Babystep_Ctrl, nullptr),
  VPHELPER(VP_TEMP_Mode_L, nullptr, ScreenHandler.TempModeChange, nullptr),
  VPHELPER(VP_TEMP_Mode_R, nullptr, ScreenHandler.TempModeChange, nullptr),
  ...
  VPHELPER(VP_DetectorFil_L_SW, &DGUSDetectorFil_L, ScreenHandler.GetTurnOffCtrl, nullptr),
  VPHELPER(VP_DetectorFil_R_SW, &DGUSDetectorFil_R, ScreenHandler.GetTurnOffCtrl, nullptr),
  VPHELPER(VP_TEMP_Protect_SW, &DGUSTEMP_Protect, ScreenHandler.GetTurnOffCtrl, nullptr),
  VPHELPER(VP_PowerLossRec_SW, &recovery.enabled, ScreenHandler.GetTurnOffCtrl, nullptr),
  ...
  // SDCard File listing
    ...
    VPHELPER(VP_SD_Print_Setting, nullptr, ScreenHandler.DGUSLCD_SD_PrintTune, nullptr),
      ...
      VPHELPER(VP_SD_Print_LiveAdjustX, nullptr, ScreenHandler.HandleLiveAdjustZ, ScreenHandler.DGUSLCD_SendFloatAsIntValueToDisplay<2>),
      VPHELPER(VP_SD_Print_LiveAdjustY, nullptr, ScreenHandler.HandleLiveAdjustZ, ScreenHandler.DGUSLCD_SendFloatAsIntValueToDisplay<2>),
      ...
      VPHELPER(VP_OFFSET_X2, &hotend_offset[1].x, ScreenHandler.GetOffsetValue,ScreenHandler.DGUSLCD_SendFloatAsLongValueToDisplay<2>),
      VPHELPER(VP_OFFSET_Y2, &hotend_offset[1].y, ScreenHandler.GetOffsetValue,ScreenHandler.DGUSLCD_SendFloatAsLongValueToDisplay<2>),
      VPHELPER(VP_OFFSET_Z2, &hotend_offset[1].z, ScreenHandler.GetOffsetValue,ScreenHandler.DGUSLCD_SendFloatAsLongValueToDisplay<2>),
    #endif
  #else
  ...
  // mixware
    ...
    VPHELPER(VP_Chamber_CONTROL, &thermalManager.temp_chamber.target, ScreenHandler.HandleHeaterControl, nullptr),
    VPHELPER(VP_Chamber_STATUS, &thermalManager.temp_chamber.target, nullptr, ScreenHandler.DGUSLCD_SendHeaterStatusToDisplay),
  #endif

  VPHELPER(VP_EXT_L_MANUAL, nullptr, ScreenHandler.HandleManualTemperature, nullptr),
  VPHELPER(VP_EXT_R_MANUAL, nullptr, ScreenHandler.HandleManualTemperature, nullptr),
  VPHELPER(VP_BED_MANUAL, nullptr, ScreenHandler.HandleManualTemperature, nullptr),
  VPHELPER(VP_CHAMBER_MANUAL, nullptr, ScreenHandler.HandleManualTemperature, nullptr),
  VPHELPER(VP_FAN_L_MANUAL, nullptr, ScreenHandler.HandleManualTemperature, nullptr),
  VPHELPER(VP_FAN_R_MANUAL, nullptr, ScreenHandler.HandleManualTemperature, nullptr),
  VPHELPER(VP_FAN_C_MANUAL, nullptr, ScreenHandler.HandleManualTemperature, nullptr),
  VPHELPER(VP_LIGHT_MANUAL, nullptr, ScreenHandler.HandleManualTemperature, nullptr),

  VPHELPER(VP_X_JERK_SPEED, (uint16_t *)&planner.max_jerk.x, ScreenHandler.HandleJerk_MKS, ScreenHandler.DGUSLCD_SendWordValueToDisplay),
  VPHELPER(VP_Y_JERK_SPEED, (uint16_t *)&planner.max_jerk.y, ScreenHandler.HandleJerk_MKS, ScreenHandler.DGUSLCD_SendWordValueToDisplay),
  VPHELPER(VP_Z_JERK_SPEED, (uint16_t *)&planner.max_jerk.z, ScreenHandler.HandleJerk_MKS, ScreenHandler.DGUSLCD_SendWordValueToDisplay),
  VPHELPER(VP_E0_JERK_SPEED, (uint16_t *)&planner.max_jerk.e, ScreenHandler.HandleExtruderJerk_MKS, ScreenHandler.DGUSLCD_SendWordValueToDisplay),
  VPHELPER(VP_E1_JERK_SPEED, (uint16_t *)&planner.max_jerk.e, ScreenHandler.HandleExtruderJerk_MKS, ScreenHandler.DGUSLCD_SendWordValueToDisplay),
