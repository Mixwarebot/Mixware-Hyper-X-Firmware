...
FORCE_INLINE void segment_idle(millis_t &next_idle_ms) {
  ...
  float x_home_pos(const uint8_t extruder) {
    if (extruder == 0)
      return X_HOME_POS - probe.offset.x;
    else
      /**
    ...
    if (active_extruder_parked) {
      switch (dual_x_carriage_mode) {
        ...
        case DXC_AUTO_PARK_MODE: {
          ...
          const feedRate_t fr_zfast = PLANNER_XY_FEEDRATE()/10;//planner.settings.max_feedrate_mm_s[Z_AXIS];
          ...
          if (planner.buffer_line(raised, fr_zfast)) {
            ...
            if (planner.buffer_line(curpos, fr_zfast)) {//PLANNER_XY_FEEDRATE()
              //  3. Lower Z back down
        ...
        case DXC_DUPLICATION_MODE:
          if (active_extruder == 0) {
            ...
            if (!planner.buffer_line(new_pos, MMM_TO_MMS(XY_PROBE_FEEDRATE), 1)) break;
            planner.synchronize();
...
void set_axis_is_at_home(const AxisEnum axis) {
   ...
   */
    ...
    if (axis == Z_AXIS) {
      #if (HOMING_Z_WITH_PROBE || ENABLED(TFT_MIXWARE_LVGL_UI) || ENABLED(DGUS_LCD_UI_MIXWARE))
  ...
  if (axis == Y_AXIS) {
    if (active_extruder == 0) {
      current_position.y -= probe.offset.y;
    }
  }
